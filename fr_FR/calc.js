  ...
  .controller('splatController', ['$scope', '$rootScope', '$timeout', '$locale', function splatCtrl($scope, $rootScope, $timeout, $locale, $translate, $uibModal, $log) {
    ...
    $scope.calcStat = function(abilityScore, weaponType, stat, saveStat) {
      ...
      if(stat == "STAT_RUN_SPEED_RESIST") {
        ...
        var name = "Vitesse à pieds (Encre ennemie)";
      ...
      if(stat == "STAT_RUN_SPEED_FIRING") {
        ...
        if($scope.loadout.weapon.class.toLowerCase() == 'brush') {
            ...
            var name = "[+] Vitesse max en roulant";
            ...
            var desc = "La vitesse des épinceaux est affecté par Arme principale +.";
        ...
        if($scope.loadout.weapon.class.toLowerCase() == 'splatling' || $scope.loadout.weapon.class.toLowerCase() == 'brella') {
          var name = "[+] Vitesse à pieds (En tirant)";
        }
        else {
          var name = "Vitesse à pieds (En tirant)";
        }
        ...
        var percentage = delta;
        var label = "{value} DU/f".format({value: $scope.toFixedTrimmed(value,4)});
        var desc = "Unités de distance/frame";
      ...
      if(stat == "STAT_RUN_SPEED_CHARGING") {
        ...
        var name = "[+] Vitesse à pieds (En chargeant)";
        var value = run_speed;
        var percentage = delta;
      ...
      if(stat == "STAT_RUN_SPEED_FLICKING_HORIZONTAL") {
        ...
        var name = "[+] Vitesse à pieds : Coup Horizontal";
        var value = run_speed;
        var percentage = delta;
        var label = "{value} DU/f".format({value: $scope.toFixedTrimmed(run_speed,4)});
        var desc = "Unités de distance/frame";        
      ...
      if(stat == "STAT_RUN_SPEED_FLICKING_VERTICAL") {
        ...
        var name = "[+] Vitesse à pieds : Coup Vertical";
        var value = run_speed;
        var percentage = delta;
        var label = "{value} DU/f".format({value: $scope.toFixedTrimmed(run_speed,4)});
        var desc = "Unités de distance/frame";
      ...
      if(stat == "STAT_RUN_SPEED_FLICKING") {
        ...
        var name = "[+] Vitesse à pieds (En secouant)";
        var value = run_speed;
        var percentage = delta;
        var label = "{value} DU/f".format({value: $scope.toFixedTrimmed(run_speed,4)});
        var desc = "Unités de distance/frame";
      ...
      if(stat == "STAT_SPECIAL_CHARGE") {
        ...
        var name = "Vitesse de charge du spécial";
        var value = special_charge_speed;
        var percentage = ((special_charge_speed*100) - 100).toFixed(1);
        var desc = "{value}p pour le spécial".format({value: Math.ceil($scope.loadout.weapon.specialCost / special_charge_speed)});
      ...
      if(stat == "STAT_SAVER_MAIN") {
        ...
        if($scope.loadout.weapon.class == "Roller" || $scope.loadout.weapon.class == "Brush") {
          ...
          var name = "[+] Cons. d'encre (Pr.): En roulant";
          var desc = "{totalShots} pour vider ({reduction}% réduction)".format({totalShots: Math.floor(100/costPerShot), reduction: (100 - (reduction*100)).toFixed(1)});       
          var label = "{value}/seconde".format({value: $scope.toFixedTrimmed(costPerShot,3)});
        }
        else {
          var costPerShot = $scope.loadout.weapon.inkPerShot * reduction;
          var name = "Consommation d'encre (Principal)";
          var desc = "{totalShots} pour vider ({reduction}% réduction)".format({totalShots: Math.floor(100/costPerShot), reduction: (100 - (reduction*100)).toFixed(1)});       
          var label = "{value}% réservoir/{unit}".format({value: $scope.toFixedTrimmed(costPerShot,3), unit: $scope.loadout.weapon.shotUnit});
        }
  
        if($scope.loadout.weapon.name.indexOf("Splattershot Jr.") !== -1) {
          var desc = "{totalShots} pour vider ({reduction}% réduction)".format({totalShots: Math.floor(110/costPerShot), reduction: (100 - (reduction*100)).toFixed(1)});                
        }
      ...
      if(stat == "STAT_SAVER_MAIN_FLICK") {
        ...
        var name = "[+] Consommation d'encre (Principal)";
        var desc = "{totalShots} pour vider ({reduction}% réduction)".format({totalShots: Math.floor(100/costPerShot), reduction: (100 - (reduction*100)).toFixed(1)});       
        var label = "{value}% réservoir/{unit}".format({value: $scope.toFixedTrimmed(costPerShot,3), unit: "coup"});
        var value = costPerShot;
        var percentage = (100 - (reduction*100)).toFixed(1);
      ...
      if(stat == "STAT_SAVER_MAIN_HORIZONTAL_FLICK") {
        ...
        var name = "[+] Cons. d'encre (Pr.): Horizontal";
        var desc = "{totalShots} pour vider ({reduction}% réduction)".format({totalShots: Math.floor(100/costPerShot), reduction: (100 - (reduction*100)).toFixed(1)});       
        var label = "{value}% réservoir/{unit}".format({value: $scope.toFixedTrimmed(costPerShot,3), unit: "coup horizontal"});
        var value = costPerShot;
        var percentage = (100 - (reduction*100)).toFixed(1);
      ...
      if(stat == "STAT_SAVER_MAIN_VERTICAL_FLICK") {
        ...
        var name = "[+] Cons. d'encre (Pr.): Vertical";
        var desc = "{totalShots} pour vider ({reduction}% réduction)".format({totalShots: Math.floor(100/costPerShot), reduction: (100 - (reduction*100)).toFixed(1)});       
        var label = "{value}% réservoir/{unit}".format({value: $scope.toFixedTrimmed(costPerShot,3), unit: "coup vertical"});
        var value = costPerShot;
        var percentage = (100 - (reduction*100)).toFixed(1);
      ...
      if(stat == "STAT_SAVER_SUB") {
        ...
        var name = "Consommation d'encre (Secondaire)";
        var desc = "{reduction}% réduction".format({reduction: (100 - (reduction*100)).toFixed(1)})
        var label = "{value}% réservoir".format({value: $scope.toFixedTrimmed(costPerSub,3)})      
      ...
      if(stat == "STAT_RECOVERY_SQUID") {
        ...
        var name = "Vitesse de recharge d'encre (Calamar)";
        var value = delta;
        var percentage = (100 - (100 / delta) * 100).toFixed(1);
        var desc = "{value}s de vide à plein".format({value: refill_time.toFixed(2)})
      ...
      if(stat == "STAT_RECOVERY_KID") {
        ...
        var name = "Vitesse de recharge d'encre (Enfant)";
        var value = delta;
        var percentage = (100 - (100 / delta) * 100).toFixed(1);
        var desc = "{value}s de vide à plein".format({value: refill_time.toFixed(2)})      
        var label = "{value}s".format({value: refill_time.toFixed(2)})
    ...
    $scope.statBarClicked = function(name) {
      ...
      if($scope.loadout.weapon.class.toLowerCase() == 'brush') {
         ...
         * INK SAVER (MAIN) STATS *
         **************************/
        if(name == "[+] Consommation d'encre (Principal)") {
          var abilityScore = $scope.loadout.calcAbilityScore('Ink Saver (Main)');
