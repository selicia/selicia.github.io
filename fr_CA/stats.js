...
angular.module('splatApp').stats = function ($scope) {
  $scope.stats = {
    ...
    'Ink Recovery Speed (Squid)': new Stat("Vitesse de recharge d'encre (Calamar)", function(loadout) {
      ...
      this.desc = "{value}s de vide à plein".format({value: seconds.toFixed(2)})
    ...
    'Ink Consumption (Main)': new Stat("Consommation d'encre (Principal)", function(loadout) {
      ...
      this.desc = "{totalShots} pour vider ({reduction}% reduction)".format({totalShots: Math.floor(100/costPerShot), reduction: (reduction*100).toFixed(1)})
      this.label = "{value}% réservoir/{unit}".format({value: costPerShot.toFixed(1), unit: loadout.weapon.shotUnit})
