  ...
  $scope.stats = {
    ...
    'Run Speed (Firing)': new Stat("ヒト移動速度 (発射中)", function(loadout) {
        ...
        if(loadout.weapon.class.toLowerCase() == 'brush') {
            ...
            this.desc = "ブラシスピードはメインパワーアップの影響を受けます。";
            return this.value;
        }

        if(loadout.weapon.class.toLowerCase() == 'charger') {
          this.name = "ヒト移動速度 (ダッシュ)";
        }
    ...
    'Ink Recovery Speed (Squid)': new Stat("インク回復力 (イカ)", function(loadout) {
      ...
      this.desc = "満タンから空まで{value}秒".format({value: refill_time.toFixed(2)})
    ...
    'Ink Consumption (Main)': new Stat("メインウェポンの消費インク量", function(loadout) {
      ...
      if(loadout.weapon.class == "Roller" || loadout.weapon.class == "Brush") {
        ...
        this.name = "[+] メインウェポンの消費インク量：ローリング";
        ...
        this.label = "一秒に{value}インク消費".format({value: $scope.toFixedTrimmed(costPerShot,3)});
      }
      else {
      ...
      if(loadout.weapon.name.indexOf("Splattershot Jr.") !== -1) {
        this.desc = "満タンから空まで{totalShots}回 ({reduction}% 減少)".format({totalShots: Math.floor(110/costPerShot), reduction: (100 - (reduction*100)).toFixed(1)});                
      }
    ...
    'Ink Consumption (Sub)': new Stat("サブウェポンの消費インク量", function(loadout) {
      ...
      this.desc = "{reduction}% 減少".format({reduction: (100 - (reduction*100)).toFixed(1)})
      this.label = "タンクの{value}%".format({value: $scope.toFixedTrimmed(costPerSub,3)})      
    ...
    'Special Saved': new Stat("復活後のスペシャルゲージの残り", function(loadout) {
      ...
      if(loadout.hasAbility('Respawn Punisher')) {
        ...
        this.desc = "復活ペナルティアップはこのステータスに影響を与えています。";
      }
      else {
        this.desc = null;
      }
    ...
    'Special Saved On Death': new Stat("復活後のスペシャルゲージの残り (使用中の死)", function(loadout) {
    ...
    'Special Power': new Stat("スペシャル性能", function(loadout) {
      ...
      switch(equippedSpecial.name) {
        ...
        case 'Booyah Bomb':
          ...
          this.name = "スペシャル性能<br>(最大充電時間)";
          this.label = "{value}秒".format({value: $scope.toFixedTrimmed(charge_time,4)})
          return charge_time;
      }
      return 0;
    }, 100),
    ...
    'Sub Power': new Stat("サブ性能", function(loadout) {
      ...
      switch(equippedSub.name) {
        ...
        case 'Point Sensor':
          ...
          this.name = "サブ性能<br>(飛距離)";
          this.label = "{value}".format({value: $scope.toFixedTrimmed(scan_radius,2)})
          return scan_radius;
        ...
        case 'Ink Mine':
          ...
          this.label = "{value}%".format({value: $scope.toFixedTrimmed(radius*100,2)})
        ...
        case 'Sprinkler':
          ...
          this.name = "サブ性能<br>(期限)";
          this.label = "{value}秒".format({value: $scope.toFixedTrimmed(total_duration,2)})
    ...
    'Tracking Time': new Stat("マーキング時間 (ポイントセンサー)", function(loadout) {
    ...
    'Main Power Up 1': new Stat("メイン性能アップ *", function(loadout) {
         ...
         loadout.weapon.name.indexOf('Pro') == -1)
      {
        ...
        this.name = "メイン性能アップ (ジャンプショットランダム化)";      
      }
      ...
      if(loadout.weapon.name.indexOf('Hydra Splatling') != -1) {
        ...
        this.name = "メイン性能アップ (フルチャージ最小ダメージ)";      
      }
      ...
      if(parameters) {
          ...
          )
        {
          ...
          this.percentage = ((result/min_param - 1) * 100).toFixed(1);
          result = result / 60.0;
          this.label = "{value}秒".format({value: $scope.toFixedTrimmed(result,2)});
          label_set = true;
        }
    ...
    'Main Power Up 2': new Stat("メイン性能アップ *", function(loadout) {
         ...
         loadout.weapon.name.indexOf('Hero Charger Replica') != -1)
      {
        ...
        this.name = "メイン性能アップ (部分チャージ最大ダメージ)";      
      }
      ...
      if(loadout.weapon.name.indexOf('Hydra Splatling') != -1) {
        ...
        this.name = "メイン性能アップ (フルチャージ最大ダメージ)";      
      }
