...
angular.module('splatApp').controller('ModalCtrl', function($scope, $uibModal, $log, $timeout) {
  ...
  var templates = {
    ...
    <span>{{selectedWeapon.localizedName['es_ES']}}</span></div>
    <div class="col-md-12">
    <div class="row">
    <div class="col-xs-4 nopadding">
    <img ng-src="{{getSubIcon(selectedWeapon.sub)}}" uib-tooltip="{{getSubByName(selectedWeapon.sub).localizedName['es_ES']}}" tooltip-append-to-body="true"  class="subspeicon" />
    </div>
    <div class="col-xs-8 nopadding">
    <div class="subspe-bubble">
    <img ng-src="{{getSpecialIcon(selectedWeapon.special)}}" uib-tooltip="{{getSpecialByName(selectedWeapon.special).localizedName['es_ES']}}" tooltip-append-to-body="true" class="subspeicon" />
    ...
    <div class="row">
    <div class="col-md-12">
    <select class="form-control dropdown-toggle" data-ng-options="x.localizedName['es_ES' || 'es_ES'] for x in weaponSets" data-ng-model="selectedSet" ng-change="switchSet()"></select>
    </div>
    </div>
    <div class="col-md-12">
    <div class="row">
    ...
    <img class="gear-icon" ng-src="{{::weapon.image}}" ng-click="selectWeapon(weapon)" uib-tooltip="{{::weapon.localizedName['es_ES']}}" tooltip-append-to-body="true"/>
    </div>
    </div>
    </div>
    </div>
    ...
    <button class="btn" type="button" onclick="animateButton(this)" ng-click="cancel()"><span>Cancelar</span></button>
    </div>
    ...
    <span>{{selectedGear.localizedName['es_ES']}}</span></div>
    <div id="gearpicker-stats">
    <span ng-if="selectedGear.main != undefined"><img ng-src="{{getSkillByName(selectedGear.main).image}}"/>  {{getSkillByName(selectedGear.main).localizedName['es_ES']}}</span>
    <span ng-if="selectedGear.main == undefined"><img ng-src="../common/assets/img/skills/Unknown.png"/>  ???</span>
    <br>
    <img ng-src="{{brands[selectedGear.brand].image}}"/> {{brands[selectedGear.brand].localizedName['es_ES']}}<br>
    <div>
    ...
    <div class="col-md-8 picker-right">
    <div class="picker">
    <div ng-click="selectGear(item)"  ng-repeat="item in filtered.primary track by item.id" uib-tooltip="{{::item.localizedName['es_ES']}}" tooltip-append-to-body="true" class="gear-wrapper">
    ...
    --><div ng-click="selectGear(item)" ng-repeat="item in filtered.secondary track by item.id" uib-tooltip="{{::item.localizedName['es_ES']}}" tooltip-append-to-body="true" class="gear-wrapper">
    <img class="gear-icon" ng-src="{{::item.image}}"/>
    <span class="brand-icon">
    <img ng-src="{{::brands[item.brand].image}}"/>
    </span>
    ...
    <span class="annie">
    <img ng-if="item.brand != 'Grizzco'?item.name != 'Splatfest Tee'?true:false:false" src="../common/assets/img/misc/annie.png" tooltip-append-to-body="true" tooltip-placement="bottom" uib-tooltip="Solo disponible en SplatNet2"/>
    </span>
    </div><!--
    ...
    <span class="not-possible" uib-tooltip="No es posible obtenerlo con este potenciador" tooltip-append-to-body="true">
    ...
    <div class="row buttons">
    <div class="col-xs-12">
    <button class="btn" type="button" onclick="animateButton(this)" ng-click="ok()"><span>¡Vale!</span></button>
    </div>
    </div>
    </div>
    </div>
    ...
    <p>Creado principalmente con AngularJS 1.6.5 y Bootstrap 3.<br><br> Esta herramienta fue desarrollada utilizando información y recursos extraídos de <a href="https://twitter.com/LeanYoshi" target="_blank">Lean</a>, así como de información proporcionada por varios miembros de Inkademy y de los servidores de desarrolladores de Splatoon en Discord. <br><br>¡Agradecimientos a todos los que me prestaron su ayuda e información!<br><br> Muchas de las fórmulas usadas puedes verlas en <a href="https://splatoonwiki.org/wiki/User:Heddy/Charts" target="_blank">Heddy's charts</a> en Inkipedia.<br><br> Puedes dejar cualquier sugerencia, informe de error o comentario en el apartado "issues" de la página <a href="https://github.com/DeviPotato/splat2-calc" target="_blank">GitHub</a>. <br> del proyecto También puedes contactarme a través del <a href="https://twitter.com/loadout_ink" target="_blank">twitter</a>.</p>
    <p>
