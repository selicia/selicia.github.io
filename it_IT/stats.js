...
angular.module('splatApp').stats = function ($scope) {
  $scope.stats = {
    'Swim Speed': new Stat("Velocità in nuoto", function(loadout) {
      ...
      this.desc = "Unità Distanza/Frame";
      console.log("SPEED: " + speed.toFixed(4));
      return speed.toFixed(3);
    }, 2.4),
    'Run Speed': new Stat("Velocità di corsa", function(loadout) {
    ...
    'Run Speed (Enemy Ink)': new Stat("Vel. Corsa (Inchiostro nemico)", function(loadout) {
    ...
    'Run Speed (Firing)': new Stat("Vel. Corsa (Sparando)", function(loadout) {
        ...
        if(isNaN(this.value)) {
          this.value = 0;
          this.label = "Non funzionante";
          this.desc = null;
        }
        return this.value.toFixed(1);
      }, 1.44),
    'Ink Recovery Speed (Squid)': new Stat("Vel. recupero inchiostro (Calamaro)", function(loadout) {
      var abilityScore = loadout.calcAbilityScore('Ink Recovery Up');
      var seconds = 3 * (1 - this.calcMod(abilityScore) / (600 / 7))
      this.desc = "{value}s da vuoto a pieno".format({value: seconds.toFixed(2)})
      this.value = ((3 / seconds) * 100)
      this.label = "{value}%".format({value: this.value.toFixed(1)})
      return this.value.toFixed(1);
    }, 154),
    'Ink Recovery Speed (Kid)': new Stat("Vel. recupero inchiostro (Ragazzo)", function(loadout) {
      var abilityScore = loadout.calcAbilityScore('Ink Recovery Up');
    ...
    'Ink Consumption (Main)': new Stat("Consumo inchiostro (Principale)", function(loadout) {
      var abilityScore = loadout.calcAbilityScore('Ink Saver (Main)');
      this.name = "Consumo inchiostro (Principale)"
      var coeff = (200 / 3)
      ...
      this.desc = "Vuoto in {totalShots} ({reduction}% riduzione)".format({totalShots: Math.floor(100/costPerShot), reduction: (reduction*100).toFixed(1)})
      this.label = "{value}% tanica/{unit}".format({value: costPerShot.toFixed(1), unit: loadout.weapon.shotUnit})
    ...
    'Ink Consumption (Sub)': new Stat("Consumo inchiostro (Secondaria)", function(loadout) {
      var abilityScore = loadout.calcAbilityScore('Ink Saver (Sub)');
      this.name = "Consumo inchiostro (Secondaria)"
      var coeff = (600 / 7)
      ...
      this.desc = "{reduction}% riduzione".format({reduction: reduction.toFixed(1)})
      this.label = "{value}% tanica".format({value: this.value.toFixed(2)})
      return costPerSub;
    }, 100),
    'Special Charge Speed': new Stat("Vel. caricamento speciale", function(loadout) {
      ...
      this.desc = "{value}p per la speciale".format({value: Math.round(loadout.weapon.specialCost / chargeSpeed)})
      this.label = "{value}%".format({value: (this.value*100).toFixed(1)});
      return (chargeSpeed * 100).toFixed(1);
    }, 1.3),
    'Special Saved': new Stat("Risparmio speciale *", function(loadout) {
      ...
      if(loadout.hasAbility('Respawn Punisher')) {
        kept -= .225;
        this.desc = "Gli effetti del Castigo non sono ancora completamente chiari.";
      }
      this.value = kept;
      this.label = "{value}%".format({value: (this.value*100).toFixed(1)});
...
//TODO: clean this up a bit
    'Special Power': new Stat("Potere speciale", function(loadout) {
      ...
      switch(equippedSpecial.name) {
        ...
        case 'Splat-Bomb Launcher':
          ...
          base = 360;
          this.max = 8.1;
          this.name = "Potere speciale (Durata)"
        ...
        case 'Baller':
          ...
          base = 400;
          this.max = 600;
          this.name = "Potere speciale (Resistenza Cromosfera)"
        ...
        case 'Tenta Missiles':
          ...
          this.max = 8;
          this.max = '166'
          this.name = "Potere speciale (Dimensione reticolo)"
        ...
        case 'Splashdown':
          ...
          base = 110;
          this.max = 1.274;
          this.name = "Potere speciale (Diametro splash)"
          results = (1 +this.calcMod(abilityScore) / coeff)
          this.desc = "{value} Unità Distanza".format({value: (base*results).toFixed(1)})
          this.value = results;
          this.label = "{value}%".format({value: (results*100).toFixed(1)})
          return (results*100).toFixed(1);
          break;
        case 'Bubble Blower':
          this.name = "Potere speciale (Raggio delle bolle)";
          this.value = 0;
          this.label = "Non funzionante";
          break;
      }
...
//TODO: get effects for all subs
    'Sub Power': new Stat("Potere secondario", function(loadout) {
      ...
      switch(equippedSub.name) {
        ...
        case 'Toxic Mist':
          ...
          break;
        case 'Curling Bomb':
          this.name = "Potere secondario (Velocità della bomba)";
          this.label = "Non funzionante";
          break;
        case 'Ink Mine':
          this.name = "Potere secondario (Raggio della mina)";
          this.label = "Non funzionante";
          break;
        case 'Splash Wall':
          this.name = "Potere secondario (Resistenza del muro)";
          this.label = "Non funzionante";
          ...
          break;
        case 'Sprinkler':
          this.name = "Potere secondario (Durata a piena potenza)";
          this.label = "Non funzionante";
          break;
        case 'Squid Beakon':
          this.name = "Potere secondario (Velocità di salto al trasportatore)";
          this.label = "Non funzionante";
          break;
      }
      return (range * 100).toFixed(1);
    }, 150),
    'Super Jump Time (Squid)': new Stat("Tempo super salto (Calamaro) *", function(loadout) {
    ...
    'Super Jump Time (Kid)': new Stat("Tempo super salto (Ragazzo) *", function(loadout) {
      var abilityScore = loadout.calcAbilityScore('Quick Super Jump');
      var mod = this.calcMod(abilityScore)
    ...
    'Quick Respawn Time': new Stat("Tempo rientro rapido", function(loadout) {
      var abilityScore = loadout.calcAbilityScore('Quick Respawn');
      this.name = "Tempo rientro rapido";
      this.desc = "Tempo di rientro quando splattato senza splattare nessuno.";
      var death = 30;
      var splatcam = 354;
      var spawn = 120;
    ...
    'Tracking Time': new Stat("Tempo di rilevamento *", function(loadout) {
      ...
      this.label = "{value}s".format({value: this.value.toFixed(2)});
      this.desc = "Durata del rilevamento";
