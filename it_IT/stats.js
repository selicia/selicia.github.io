...
angular.module('splatApp').stats = function ($scope) {
  $scope.stats = {
    ...
    'Ink Recovery Speed (Squid)': new Stat("Vel. recupero inchiostro (Calamaro)", function(loadout) {
      ...
      this.desc = "{value}s da vuoto a pieno".format({value: refill_time.toFixed(2)})
    ...
    'Ink Consumption (Main)': new Stat("Consumo inchiostro (Principale)", function(loadout) {
      ...
      this.desc = "Vuoto in {totalShots} ({reduction}% riduzione)".format({totalShots: Math.floor(100/costPerShot), reduction: (100 - (reduction*100)).toFixed(1)})
      this.label = "{value}% tanica/{unit}".format({value: $scope.toFixedTrimmed(costPerShot,3), unit: loadout.weapon.shotUnit})
