...
angular.module('splatApp').stats = function ($scope) {
  $scope.stats = {
    ...
    'Ink Recovery Speed (Squid)': new Stat("回墨速度 (墨魚)", function(loadout) {
      ...
      this.desc = "由空到滿需要{value}秒".format({value: seconds.toFixed(2)})
    ...
    'Ink Consumption (Main)': new Stat("墨水消耗 (主武器)", function(loadout) {
      ...
      this.desc = "射{totalShots}下就會無墨 (慳咗{reduction}%)".format({totalShots: Math.floor(100/costPerShot), reduction: (reduction*100).toFixed(1)})
      this.label = "{value}% 箱/{unit}".format({value: costPerShot.toFixed(1), unit: loadout.weapon.shotUnit})
